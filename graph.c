...
void InitGragh_List(ListGraph *G, GraphType e) {
    G->type = e;
    G->arcnum = 0;
    G->vexnum = 0;
}
...
//打印图 G
void DisplayGraph_List(ListGraph *G) {
    printf(G->vexnum>1?"Graph G with %d nodes:\n":"Graph G with %d node:\n");
    ArcNode *p;
    for (int i = 0; i < G->vexnum; i++) {
...
成功返回关键路径的长度, 失败返回 -1
*/
int CriticalPathInGragh_List(ListGraph *G, Edge cEdge[]) {
    if (G->type == UDG) {
        return -1;
    }
    int n = 0, u, v;
...
//函数 DisplayPathAndDistance_List_pro 的辅助函数
static void DisPath_List(ListGraph *G, int path[][G->vexnum], int i, int j) {
    if (path[i][j] == -1) {
        return ;
    }
    DisPath_List(G, path, i, path[i][j]);
    printf(" %c ->", G->vertex[path[i][j]]);
    DisPath_List(G, path, path[i][j], j);
}
//显示结点间最短路径与距离
void DisplayPathAndDistance_List_pro(ListGraph *G, int distance[][G->vexnum], int path[][G->vexnum]) {
    for (int i = 0; i < G->vexnum; i++) {
        for (int j = 0; j < G->vexnum; j++) {
...
成功返回 true, 失败返回 false
*/
bool InsertArc_Martix(MatrixGraph *G, VertexType v, VertexType w, int info) {
...
//打印图 G
void DisplayGraph_Matrix(MatrixGraph *G) {
    printf("Matrix Graph:\n  |");
    for (int i = 0; i < G->vexnum; i++) {
...
//函数 DisplayPathAndDistance_Matrix_pro 的辅助函数
static void DisPath_Matrix(MatrixGraph *G, int path[][G->vexnum], int i, int j) {
    if (path[i][j] == -1) {
        return ;
    }
    DisPath_Matrix(G, path, i, path[i][j]);
    printf(" %c ->", G->vertex[path[i][j]]);
    DisPath_Matrix(G, path, path[i][j], j);
}
//显示结点间最短路径与距离
void DisplayPathAndDistance_Matrix_pro(MatrixGraph *G, int distance[][G->vexnum], int path[][G->vexnum]) {
    for (int i = 0; i < G->vexnum; i++) {
        for (int j = 0; j < G->vexnum; j++) {
            if (distance[i][j] == MAX_ARC_INFO+1 || i == j) {
                if (i != j) {
...
int main(void) {
    ...
    InsertArc_Martix(&g, 'E', 'D', 12);
    InsertArc_Martix(&g, 'E', 'C', 17);
    //InsertArc_Martix(&g, 'D', 'C', 21);
    //DeleteArc_Matrix(G, 'D', 'C');
    InsertArc_Martix(&g, 'D', 'B', 14);
    InsertArc_Martix(G, 'C', 'B', 7);
    InsertArc_Martix(G, 'B', 'A', 16);
    InsertArc_Martix(G, 'A', 'E', 20);
    DisplayGraph_Matrix(&g);
    printf("%d\n", G->arcnum);
    int n = G->vexnum-1;
    /*
...
*/
    ...
    DisplayPathAndDistance_Matrix_pro(G, dis, path);
    /*
    Edge cEdge[G->arcnum];
